...
class _EditRoomPageState extends State<EditRoomPage> {
  ...
  late TextEditingController _yourGmailController;

  // Bill fields
  late TextEditingController _totalRentController;
  ...
  void _initializeControllers() {
    ...
    _yourGmailController = TextEditingController();
    _totalRentController = TextEditingController();
  }

  void _loadRoomData() {
    // Load basic info
    _roomNameController.text = widget.roomData['roomName'] ?? '';
    _locationController.text = widget.roomData['location'] ?? '';
    _yourNameController.text = widget.roomData['creatorName'] ?? '';
    _yourGmailController.text = widget.roomData['creatorEmail'] ?? '';

    // Load bills
    final bills = widget.roomData['bills'] as Map<String, dynamic>? ?? {};
    _totalRentController.text = (bills['totalRent'] ?? 0).toString();

    // Load food bills
    final foodBills = bills['food'] as List? ?? [];
    for (var bill in foodBills) {
      _foodFields.add({
        'name': TextEditingController(text: bill['name'] ?? ''),
        'amount': TextEditingController(text: (bill['amount'] ?? 0).toString()),
      });
    }
    if (_foodFields.isEmpty) _addFoodField();

    // Load utility bills
    final utilityBills = bills['utilities'] as List? ?? [];
    for (var bill in utilityBills) {
      _utilityFields.add({
        'name': TextEditingController(text: bill['name'] ?? ''),
        'amount': TextEditingController(text: (bill['amount'] ?? 0).toString()),
      });
    }
    if (_utilityFields.isEmpty) _addUtilityField();

    // Load other bills
    final otherBills = bills['other'] as List? ?? [];
    for (var bill in otherBills) {
    ...
    // Load users
    final users = widget.roomData['users'] as List? ?? [];
    for (var user in users) {
      if (user['role'] != 'creator') {
        _userFields.add({
          'name': TextEditingController(text: user['name'] ?? ''),
          'gmail': TextEditingController(text: user['gmail'] ?? ''),
        });
      }
    }
    if (_userFields.isEmpty) _addUserField();
  }
  ...
  Widget build(BuildContext context) {
    // We'll reuse the same UI as CreateRoomPage
    // Import the widgets from createroom.dart or copy them here
    return Scaffold(
      appBar: AppBar(
        title: const Text(
          'Edit Room',
  ...
  // Helper widgets (same as CreateRoomPage)
